...
def read_autophase_data1d(file_path: Path) -> Tuple[dict, List]:
    ...
    Returns:
        Tuple: A tuple containing the dictionary (dic) and the autophased fid_decay.
    """
...
def fft_autophase(file_path: Path, fid_decay: ndarray) -> ndarray:
...
def integrate_2d(
    data_2d: ndarray, ppm_scale: ndarray, ppm_start: float, ppm_end: float
) -> ndarray:
...
def load_t1_ir_t2_timedat_files(file_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    time_t1_path = data1d_path / "time_t1_s.dat"
    time_t2_path = data1d_path / "time_t2_s.dat"
